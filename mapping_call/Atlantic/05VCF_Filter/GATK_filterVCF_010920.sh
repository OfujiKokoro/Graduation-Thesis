...
VCF_DIR="../04GATK_Call/SelectVariants"
CHR_LIST="rename_chr_noX.txt"
METRICS_DIR="../03MarkDuplicate/bam_DP"
python_script="filterVCF_010920.py"

for vcf_file in "$VCF_DIR"/*.vcf.gz; do
  sample_name=$(basename "$vcf_file" .TrimAlt.vcf.gz)
  metrics_file="$METRICS_DIR/${sample_name}_depth_metrics.txt"
